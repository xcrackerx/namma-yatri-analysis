...
-- check whether there is any duplicate tripid
...
SELECT SUM(searches_for_quotes) quotes FROM trips_details;
-- 1455

-- total searches which got quotes
SELECT SUM(searches_got_quotes) fare_quotes FROM trips_details;
-- 1277

-- total customer not cancelled
SELECT SUM(customer_not_cancelled) cnot FROM trips_details;
-- 1120

-- total driver not cancelled
SELECT SUM(driver_not_cancelled) dnot FROM trips_details;
-- 1140

-- total otp entered
SELECT SUM(otp_entered) otp FROM trips_details;
-- 983
...
SELECT COUNT(*) - SUM(driver_not_cancelled) dyes FROM trips_details;
-- 1021

-- cancelled bookings by customer
SELECT COUNT(*) - SUM(customer_not_cancelled) cyes FROM trips_details;
-- 1041
...
SELECT SUM(fare)/COUNT(*) average_fare FROM trips;
-- 764.3367

-- distance travelled
SELECT SUM(distance) dist FROM trips;
-- 14148 
...
FROM 
(SELECT loc_from, loc_to, COUNT(DISTINCT tripid) trips FROM trips
...
FROM
(SELECT duration, COUNT(DISTINCT tripid) trips FROM trips
...
SELECT SUM(searches_got_estimate)*100/SUM(searches) estimate FROM trips_details;
-- 81.3512

-- estimate to search for quote rates
SELECT SUM(searches_for_quotes)*100/SUM(searches_got_estimate) estimate FROM trips_details;
-- 82.7645

-- quote acceptance rate
SELECT SUM(searches_got_quotes)*100/SUM(searches_for_quotes) estimate FROM trips_details;
-- 87.7663

-- quote to booking rate
SELECT SUM(customer_not_cancelled)*100/SUM(searches_got_quotes) estimate FROM trips_details;
-- 87.7056

-- booking cancellation rate
SELECT (COUNT(*) - SUM(driver_not_cancelled))*100/SUM(customer_not_cancelled) estimate FROM trips_details;
-- 91.1607

-- conversion rate
SELECT SUM(end_ride)*100/SUM(searches) estimate FROM trips_details;
-- 45.4882
...
SELECT * FROM
(SELECT *, RANK() OVER(ORDER BY c_cancel DESC) rnk
FROM
(SELECT loc_from, COUNT(*) - SUM(customer_not_cancelled) c_cancel FROM trips_details 
GROUP BY loc_from)a)b
WHERE rnk = 1;

SELECT * FROM
(SELECT *, RANK() OVER(ORDER BY d_cancel DESC) rnk
FROM
(SELECT loc_from, COUNT(*) - SUM(driver_not_cancelled) d_cancel FROM trips_details 
GROUP BY loc_from)a)b
WHERE rnk = 1;
